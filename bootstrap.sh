...
# Every project/environment with a rendered rule file, read back from the COVERS marker
# each one carries rather than from a list someone has to remember to update. The pair
# matters, not just the project: a project bootstrapped for staging that also ships prod
# would otherwise read as covered while prod has no keystone rule at all — the same
# silent gap one level further in.
pairs="$(sed -n 's/^# COVERS: //p' "$out_dir"/project-*.yaml | sort -u)"
: "${pairs:=__none__ __none__}"
...
# any series whose project/env pair has no rendered rule file.
#
# The selector touches every series every project sends (~3k for one project). Cheap at
# this size, evaluated instantly every 5m; revisit if a project's active series reach
# five figures.
#
    ...
    rules:
      - uid: projects-uncovered
        ...
        data:
          - refId: QUERY
            ...
            model:
              ...
              instant: true
              editorMode: code
              expr: count by (project, env) ({project!="", env!=""} unless on (project, env) (${covered_expr}))
          - refId: FIRING
        ...
        labels:
          severity: warning
        annotations:
          summary: "{{ \$labels.project }}/{{ \$labels.env }} is sending telemetry but has no alert rules"
          description: "Telemetry is arriving for a project/environment bootstrap.sh was never run for, so nothing would notice if it went silent. Run ./bootstrap.sh {{ \$labels.project }} {{ \$labels.env }} on the monitoring host."
COVERAGE
