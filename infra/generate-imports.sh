...
lookup_dns_record() {
    ...
    # name, and the apply would rewrite it into a CNAME. This runs in `$(...)`, so
    # a `die` here would only exit the subshell and read as "no record": an API
    # failure gets its own status 2, which every call site distinguishes from 1.
    local body
    body="$(api "zones/$zone/dns_records?name=$hostname&type=CNAME")" || return 2
...
grafana_record="$(lookup_dns_record "grafana.$domain")" || {
    [[ $? -ne 2 ]] || die "DNS lookup for grafana.$domain failed (token lacks DNS:Read?)"
    die "no CNAME found for grafana.$domain"
}
...
for host in "otel.$domain" "otlp.$domain"; do
    if otel_record="$(lookup_dns_record "$host")"; then
        ...
        die "DNS lookup for $host failed (token lacks DNS:Read?)"
    fi
done
...
access_policies="$(api "accounts/$account/access/policies?per_page=100")"
